...
# --- POPUP AI (FIX 404 + PARSING) ---
@st.dialog("STRATEGIC ANALYSIS", width="large")
def show_details(h, a, m, sport_key):
    with st.spinner("Billy Walters sta analizzando..."):
        ...
        except: news = "Nessuna news web disponibile."
...
# --- UI PRINCIPALE ---
st.markdown('<div class="maradona-header"><h1>M4 STRATEGIC TERMINAL</h1><p>Intelligence Evolution v28.0</p></div>', unsafe_allow_html=True)
with st.sidebar:
    ...
    if st.button("🔄 SINCRONIZZA TURNO"):
        ...
        url_q = f"https://api.the-odds-api.com/v4/sports/soccer_{camp_sel.lower().replace(' ','_')}/odds/?apiKey={API_KEY_ODDS}&regions=eu&markets=h2h"
        st.session_state.live_odds = requests.get(url_q).json()
...
if 'live_data' in st.session_state and engine:
    ...
    for idx, match in enumerate(matches):
        ...
        if 'live_odds' in st.session_state:
            for mo in st.session_state.live_odds:
                if h_cl in clean_name(mo['home_team']):
                    try:
        ...
        with st.container():
            ...
            with c1: st.markdown(f"<div class='stat-container'><span class='label-header'>1 X 2</span><span class='val-p-green'>{m['1']:.0%} {m['X']:.0%} {m['2']:.0%}</span><br><span class='val-q'>{q1} | {qX} | {q2}</span></div>", unsafe_allow_html=True)
            ...
            with c6:
                st.write("<br>", unsafe_allow_html=True); st.button("🔍", key=f"ex_{idx}", on_click=show_details, args=(h_api, a_api, m, map_odds[camp_sel]))
